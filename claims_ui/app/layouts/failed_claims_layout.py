    ...
    return html.Div([
        html.Div([
            ...
            html.Div([
                ...
                html.Div([dcc.DatePickerRange(id='date-picker-range-fc', start_date_placeholder_text='Start Date', end_date_placeholder_text='End Date', display_format='YYYY-MM-DD')], className='form-element-wrapper'),
                html.Button('Apply Filters', id='apply-filters-fc-button', n_clicks=0)
            ], className="dashboard-card-content")
        ], className='dashboard-card'),

        html.Div([
            html.Div(html.H4("Failed Claims Data"), className="dashboard-card-header"),
            html.Div([dash_table.DataTable(id='failed-claims-table', columns=[], data=[])], className="dashboard-card-content")
        ], className='dashboard-card table-container'),
        
        html.Div(id='selected-failed-claim-details-fc'),
        
        html.Div([
            html.Div(html.H4("Failure Analysis by Category"), className="dashboard-card-header"),
            html.Div(dcc.Graph(id='failure-reason-bargraph-fc'), className="dashboard-card-content")
