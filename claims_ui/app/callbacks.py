...
# --- Data Query Imports ---
# Using placeholder data, so actual query functions are not called directly here.
...
def register_callbacks(app):
    ...
    def display_claim_details(active_cell, rows):
        ...
        details_layout = [
            ...
            html.P(f"Billed Amount: ${selected_row_data.get('billed_amount', 0):,.2f}"),
            html.P(f"Paid Amount: ${selected_row_data.get('paid_amount', 0):,.2f}"),
            html.P(f"Payer: {selected_row_data.get('payer', 'N/A')}"),
            html.P(f"Status: {selected_row_data.get('status', 'N/A')}"),
        ]
